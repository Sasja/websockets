...
from websockets.exceptions import InvalidStatus, NegotiationError
...
class ServerTests(EvalShellMixin, unittest.TestCase):
    ...
    def test_select_subprotocol(self):
        """Server selects a subprotocol the select_subprotocol callable."""

        def select_subprotocol(ws, client_subprotocols, server_subprotocols):
            ws.select_subprotocol_ran = True
            return ws.protocol.select_subprotocol(
                client_subprotocols,
                server_subprotocols,
            )
    ...
    def test_process_request_early_response(self):
    ...
    def test_connection_closed_during_handshake(self):
        ...
        with run_server() as server:
            with socket.create_connection(server.socket.getsockname()):
                pass
            # TODO find a better way to let the server terminate...
            time.sleep(MS)
